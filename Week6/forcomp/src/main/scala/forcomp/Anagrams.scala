...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy((c: Char) => c).map(p => (p._1.toLower, p._2.length)).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =  wordOccurrences(s.foldRight("")((str, z) => str ++ z))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
   occurrences match {
     ...
     case (char, num)::tail => {
       ...
       res.toList.map(l => l.filter(_._2 != 0))
     }
   }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsInternal(occurences: Occurrences): List[Sentence] = {
      if(occurences.isEmpty) List(Nil)
      else {
        for {
          combination <- combinations(occurences)
          word <- dictionaryByOccurrences(combination)
          sentence <- sentenceAnagramsInternal(subtract(occurences, wordOccurrences(word)))
